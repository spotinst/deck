...
//import from '@spinnaker/core' - InstanceReader, RecentHistoryService
...
module(SPOT_INSTANCE_DETAILS_INSTANCE_DETAILS_CONTROLLER, [
  UIROUTER_ANGULARJS,
  ...
  function ($scope, $state, instance, app, moniker, environment, $q) {
    ...
    function retrieveInstance() {
      ...
      if (instanceSummary && account && region && serverGroupOfInstance) {
        ...
        return ServerGroupReader.getServerGroup(app.name, account, region, serverGroupOfInstance).then((details) => {
          const serverGroupInstances = details.elastigroupActiveInstances;
          const instanceDetails = serverGroupInstances.find(
            (serverGroupInstance) => serverGroupInstance.instanceId === instance.instanceId,
          );
          let date = new Date(instanceDetails.createdAt);

          instanceSummary.launchTime = date.getTime();
          instanceSummary.account = account;
